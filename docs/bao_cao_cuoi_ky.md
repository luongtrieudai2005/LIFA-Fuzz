...
Thực nghiệm trên LightFTP (một FTP server thực tế, chạy trong môi trường Firecracker MicroVM) cho thấy các baseline không dùng LLM duy trì được khoảng 400 lần thực thi mỗi giây. Pipeline cũng xác nhận được khả năng phát hiện lỗi tràn bộ đệm thực tế thông qua AddressSanitizer. Kết quả chi tiết và những hạn chế được thảo luận trong các chương sau. Toàn bộ hệ thống gồm khoảng 17.000 dòng Python với hơn 250 bài kiểm thử.
...
| Tốc độ | Trung bình | Rất cao | Cao (trên 400 EPS) |
...
Cần nói thẳng một điều trước khi xem số liệu. Các kết quả so sánh A/B/C và câu hỏi thứ ba trong phần này được đo khi fuzzer còn một lỗi nghiêm trọng: bộ nạp mẫu không gom các gói tin trong cùng phiên lại với nhau, mỗi gói tin là một mẫu riêng nên fuzzer không bao giờ vượt qua được bước xác thực, bị kẹt ở dòng chào 220 và không chạm tới mã xử lý sau đăng nhập. Mọi so sánh A/B/C, số vùng lệnh và số lỗi từ các chiến dịch trước khi sửa lỗi này đều không còn giá trị và cần đo lại. Riêng câu hỏi thứ nhất về độ chính xác suy diễn ngữ pháp được đo độc lập với quy trình fuzzing nên vẫn còn đúng.
...
**Câu hỏi thứ hai về thông lượng.** Bảng dưới đây tóm tắt số lần gửi mỗi giây đo được trong chiến dịch dài hai giờ trên LightFTP.

Bảng 2: Thông lượng đo thực tế trên LightFTP.

| Cấu hình | Trung bình | Cao nhất | Số lần đột biến | Độ rộng | Token LLM |
|---|---|---|---|---|---|
| A ngẫu nhiên | 414 | 614 | 2.984.330 | 13.426 | 0 |
| B chỉ thống kê | 400 | 630 | 2.883.137 | 8.842 | 0 |
| C kết hợp đầy đủ | 141 | 557 | 1.012.859 | 7.689 | 55.000 |

Cột độ rộng đếm số cặp vị trí byte và giá trị duy nhất bị đột biến, tức là độ rộng của không gian đột biến chứ không phải độ phủ mã nhị phân. Cấu hình A ngẫu nhiên đột biến rộng nhất nên số này cao nhất một cách cơ học. Độ phủ giao thức thật được báo cáo riêng qua số bước chuyển trạng thái ở bảng tiếp theo.

> *[Hình 6: Biểu đồ cột so sánh thông lượng EPS của ba cấu hình. Cột A và B cao xấp xỉ nhau với trung bình 400–414 lần gửi mỗi giây. Cột C thấp hơn rõ rệt chỉ đạt 141, minh họa tác động của việc gọi LLM lên tốc độ vòng lặp chính.]*

Hai cấu hình không dùng LLM duy trì được khoảng 400 lần gửi mỗi giây, xác nhận kiến trúc bất đồng bộ giúp giữ tốc độ cao khi LLM không nằm trên đường nóng. Tuy nhiên cấu hình C có tốc độ trung bình chỉ đạt 141, thấp hơn nhiều so với kỳ vọng ban đầu. Nguyên nhân là thời gian gọi LLM và việc áp dụng bộ luật phức tạp hơn đã ảnh hưởng tới vòng lặp chính nhiều hơn thiết kế mong muốn. Để khắc phục cần cách ly triệt để hơn giữa LLM và vòng lặp nóng.

**Độ phủ trạng thái giao thức.** Bảng dưới đây báo cáo số bước chuyển trạng thái quan sát được, là bộ ba mã trạng thái trước, lệnh, mã trạng thái sau. Đây là chỉ số độ phủ giao thức đáng tin cậy nhất hiện có.

Bảng 2b: Độ phủ trạng thái giao thức trên LightFTP.

| Cấu hình | Thời gian phút | Số lần gửi | Số bước chuyển | Bước trên 1000 lần gửi |
|---|---|---|---|---|
| A ngẫu nhiên | 326 | 1.438.799 | 3.752 | 2,61 |
| B chỉ thống kê | 819 | 1.422.891 | 5.563 | 3,91 |
| C kết hợp đầy đủ | 1.154 | 1.356.229 | 2.217 | 1,63 |

Để loại trừ khác biệt về thời gian chạy, chúng tôi dùng số bước chuyển trên một nghìn lần gửi. Kết quả cho thấy cấu hình B chỉ thống kê khám phá nhiều bước chuyển nhất với 3,91, gấp khoảng 2,4 lần cấu hình C chỉ đạt 1,63. Cấu hình A ngẫu nhiên nằm giữa với 2,61. Điều đáng chú ý là cấu hình dùng LLM lại khám phá ít bước chuyển trạng thái nhất, đây là phát hiện quan trọng được phân tích ở phần 6.1. Cả ba cấu hình đều không vượt qua được trạng thái FTP đầu tiên, chỉ quan sát được mã trạng thái 220, nên các bước chuyển ở đây là chuyển nội bộ trong trạng thái chưa xác thực.

> *[Hình 7: Biểu đồ cột so sánh độ phủ trạng thái giao thức của ba cấu hình. Cột B chỉ thống kê cao nhất với 3,91 bước chuyển trên 1000 lần gửi, cột A ngẫu nhiên đạt 2,61, cột C kết hợp đầy đủ chỉ đạt 1,63. Kết quả phản trực giác này cho thấy LLM thu hẹp không gian khám phá thay vì mở rộng.]*

**Câu hỏi thứ ba về khả năng phát hiện lỗi.** Hệ thống đã xác nhận có thể phát hiện lỗi ASAN thật trên LightFTP trong một số lần chạy. Tuy nhiên trong các chiến dịch so sánh A/B/C, số lỗi duy nhất ghi nhận được không ổn định. Chiến dịch hai giờ ở bảng trên không ghi được lỗi nào trên cả ba cấu hình, trong khi một chiến dịch khác chạy cấu hình A ngẫu nhiên dài hơn chín mươi phút lại ghi tới 266 lỗi duy nhất. Sự không nhất quán này cho thấy bộ đếm lỗi hiện tại còn phụ thuộc nhiều vào điều kiện của từng phiên và chưa đủ ổn định để rút ra kết luận. Do đó câu hỏi thứ ba chưa có câu trả lời ở thời điểm báo cáo và cần phép đo có kiểm soát chặt chẽ hơn.
...
Kết quả quan trọng nhất và cũng bất ngờ nhất từ các chiến dịch thực nghiệm là cấu hình dùng LLM khám phá ít bước chuyển trạng thái giao thức hơn hẳn cấu hình chỉ dùng thống kê. Cụ thể, số bước chuyển trên một nghìn lần gửi của cấu hình B là 3,91, của A là 2,61, của C chỉ là 1,63, nghĩa là C kém B khoảng 2,4 lần. Đây là kết quả phản trực giác vì chúng tôi xây dựng cấu hình đầy đủ với kỳ vọng LLM sẽ dẫn dắt fuzzer thông minh hơn, nhưng thực tế LLM lại thu hẹp không gian khám phá trạng thái.

Nguyên nhân nằm ở cách LLM gán chiến thuật đột biến cho từng trường. Khi LLM suy luận rằng một số vị trí là magic number cố định, nó gán chiến thuật tĩnh và fuzzer bỏ qua hoàn toàn các vị trí đó. Các trường còn lại được gán tập trung vào chiến thuật giá trị biên cho trường độ dài và chiến thuật từ điển cho mã lệnh. Hậu quả là fuzzer sinh ra ít loại lệnh FTP khác nhau hơn, nó xoay quanh một tập nhỏ giá trị thay vì thử nhiều lệnh như cấu hình B chỉ dùng thống kê hay cấu hình A ngẫu nhiên. Vì đồ thị trạng thái đếm bộ ba mã trạng thái trước, lệnh, mã trạng thái sau, ít lệnh khác nhau đồng nghĩa với ít bước chuyển được khám phá.

Đây là một sự đánh đổi giữa độ chính xác và độ đa dạng. Hướng dẫn của LLM giúp tăng độ chính xác ngữ pháp, nhưng khi áp dụng lên fuzzer không cần mã nguồn có trạng thái thì lại tự hạn chế độ đa dạng trạng thái cần thiết để kích hoạt các đường xử lý sâu hơn. LLM hiểu đúng cấu trúc giao thức nhưng lại thận trọng quá, bỏ qua những đột biến có vẻ ngớ ngẩn nhưng thực ra lại là đầu dò trạng thái hiệu quả. Điều này gợi ý rằng giá trị của LLM không nên nằm ở việc gán chiến thuật quá chặt mà nên tập trung vào việc đặt tên có ý nghĩa cho các trường, trong khi quyết định đột biến cần giữ độ đa dạng nhất định, ví dụ thêm một lớp khám phá bắt buộc để bù lại số lệnh bị mất.

Cần lưu ý là bảng đo lường này đo độ phủ trạng thái giao thức chứ không đo độ phủ mã nhị phân thật, vì hệ thống hiện chưa có cơ chế phản hồi độ phủ của mã nhị phân (xem phần 6.5). Đây là sự đánh đổi chưa được báo cáo trong các công trình LLM cho fuzzing trước đây [20, 22]. Rõ ràng cần đo độ phủ mã nhị phân thực tế để xác nhận liệu cấu hình C có thực sự chạm ít đường xử lý hơn hay chỉ ít bước chuyển trạng thái hơn. Cũng cần kiểm tra liệu việc giữ đa dạng lệnh có khôi phục được độ phủ trạng thái của cấu hình C mà vẫn giữ được lợi thế về ngữ pháp. Đây là hai việc cần làm ưu tiên cho đợt đánh giá tiếp theo.

> *[Hình 8: Đồ thị thể hiện sự đánh đổi giữa độ chính xác ngữ pháp và độ đa dạng khám phá trạng thái. Trục ngang là độ đa dạng lệnh, trục dọc là số bước chuyển trạng thái trên 1000 lần gửi. Cấu hình C ở góc trên bên trái (độ chính xác cao, đa dạng thấp), cấu hình B ở góc dưới bên phải (đa dạng cao, chính xác thấp). Đường cong hình chữ L ngược cho thấy lợi ích biên của LLM giảm dần khi độ đa dạng bị thu hẹp.]*
